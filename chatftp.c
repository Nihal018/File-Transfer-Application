...
int main(int argc, char *argv[]) {
    ...
    short blockno = 0; // Initialized to 0 for the first ACK for RRQ
    int elementsWritten;
    ...
    while (1) {
        ...
        if (opcode == 3) { // DATA packet
            ...
            if (receivedBlockNo == blockno + 1) {
                ...
                memcpy(lastAckPacket + 2, &blockno, sizeof(short)); // Correctly store block number in network byte order
                lastAckPacketLen = 4;
